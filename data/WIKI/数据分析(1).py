...
#print(train_entity_knowledge_by_timestamp)
...
for head_entity, relation, tail_entity, time, key in knowledge_list:
    ...
    #print(type(head_entity)) #int
    #head_entity = str(head_entity)
    #tail_entity = str(tail_entity)


    time_points_head = correct_result_dict[str(head_entity)]
    count_head_entity = 0
    for time_point_triples in time_points_head:
        #print(time_point_triples[3])
        #print(type(time_point_triples[3]))  #int
        if int(time_point_triples[0][3]) < time:
            if any(tail_entity in triple[:1] or tail_entity in triple[2:] for triple in time_point_triples):
                count_head_entity += 1
    #print(count)
    ...
    count_tail_entity = 0
    for time_point_triples in time_points_tail:
        if int(time_point_triples[0][3]) < time:
            #print(triples)
            if any(head_entity in triple[:1] or head_entity in triple[2:] for triple in time_point_triples):
                count_tail_entity += 1
    #print(count_tail_entity)
    ...
    for time_point_triples in time_points_head:
        ...
        if int(time_point_triples[0][3]) < time:
            ...
            if any(relation == triple[1] for triple in time_point_triples):
                count_head_relation += 1
    #print(count_head_relation)
