...
# A 以文本读模式打开文件
with open('./Training_specific_knowledge_sequence(1-hop)A.json', 'r') as file:
    # 从文件中加载字典
...
'''
knowledge_list = knowledge_list[500000:]
...
for head_entity, relation, tail_entity, time, key in knowledge_list:
    ...
    for time_point_triples_head in time_points_head:
        if int(time_point_triples_head[0][3]) < time:
            if any(tail_entity in triple[:1] or tail_entity in triple[2:] for triple in time_point_triples_head):
                count_head_entity += 1
            ...
            for triple_head in time_point_triples_head:
                ...
                if head_entity in triple_head[:1] and triple_head[2] != 12554:
                    ...
                    for time_point_triples_head_1 in time_points_head_1:
                        if int(time_point_triples_head_1[0][3]) < time:
                            if any(tail_entity in triple_1[:1] or tail_entity in triple_1[2:] for triple_1 in time_point_triples_head_1):
                ...
                if head_entity in triple_head[2:] and triple_head[0] != 12554:
                    ...
                    for time_points_triples_head_2 in time_points_head_2:
                        if int(time_points_triples_head_2[0][3]) < time:
                            if any(tail_entity in triple_2[:1] or tail_entity in triple_2[2:] for triple_2 in time_points_triples_head_2):
    ...
    for time_point_triples_tail in time_points_tail:
        if int(time_point_triples_tail[0][3]) < time:
            if any(head_entity in triple[:1] or head_entity in triple[2:] for triple in time_point_triples_tail):
                count_tail_entity += 1
            ...
            for triple_tail in time_point_triples_tail:
                ...
                if tail_entity in triple_tail[:1] and triple_tail[2] != 12554:
                    ...
                    for time_point_triples_tail_1 in time_points_tail_1:
                        if int(time_point_triples_tail_1[0][3]) < time:
                            if any(head_entity in triple_1[:1] or head_entity in triple_1[2:] for triple_1 in time_point_triples_tail_1):
                ...
                if tail_entity in triple_tail[2:] and triple_tail[0] != 12554:
                    ...
                    for time_points_triples_tail_2 in time_points_tail_2:
                        if int(time_points_triples_tail_2[0][3]) < time:
                            if any(head_entity in triple_2[:1] or head_entity in triple_2[2:] for triple_2 in time_points_triples_tail_2):
    ...
    for time_point_triples_relation_head in time_points_head:
        if int(time_point_triples_relation_head[0][3]) < time:
            if any(relation == triple[1] for triple in time_point_triples_relation_head):
            ...
            for triple in time_point_triples_relation_head:
                ...
                if triple[0] !=12554 and triple[2] != 12554:
                    ...
                    for time_points_head_relation_triples1 in time_points_head_relation1:
                        if int(time_points_head_relation_triples1[0][3]) < time:
                            if any(relation == triple[1] for triple in time_points_head_relation_triples1):
                    ...
                    for time_points_tail_relation_triples1 in time_points_tail_relation1:
                        if int(time_points_tail_relation_triples1[0][3]) < time:
                            if any(relation == triple[1] for triple in time_points_tail_relation_triples1):
    ...
    if count_head_relation != 0 or count_head_relation_1_1 != 0:
        ...
        for time_point_triples_relation_tail in time_points_tail:
            if int(time_point_triples_relation_tail [0][3]) < time:
                if any(relation == triple[1] for triple in time_point_triples_relation_tail ):
                ...
                for triple in time_point_triples_relation_tail :
                    ...
                    if triple[0] != 12554 and triple[2] != 12554:
                        ...
                        for time_points_head_relation_triples2 in time_points_head_relation2:
                            if int(time_points_head_relation_triples2[0][3]) < time:
                                if any(relation == triple[1] for triple in time_points_head_relation_triples2):
                                    count_head_relation_1 += 1
                        ...
                        for time_points_tail_relation_triples2 in time_points_tail_relation2:
                            if int(time_points_tail_relation_triples2[0][3]) < time:
                                if any(relation == triple[1] for triple in time_points_tail_relation_triples2):
                                    count_tail_relation_1 += 1
